...
1. **テスト仕様書を作成**
   - 対象モジュール・関数の仕様を整理
   - 入力値・期待値・エッジケースをリストアップ
   - テスト項目ごとに TestCase を定義
...
**手順:**
1. **結合試験仕様書を作成**
   - ユースケース一覧を洗い出し（例: 「ユーザーがカートに商品を追加して購入する」）
   - 各ユースケースについて、起点→実行ステップ→終点をシナリオ化
   - データベース・API・UI の統合動作を定義

2. **結合テストコード生成と実施**
   ```bash
   # E2E テスト実行（Cypress など）
   npm run test:e2e
   
   # API 統合テスト（Supertest など）
